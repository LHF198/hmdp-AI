...
| 层次 | 技术 | 说明 |
|------|------|------|
| 后端框架 | Spring Boot 3.4.5（JDK 17） | REST API |
| 数据库 | MySQL 5.7+/8.0 + MyBatis-Plus 3.5.9 | 持久化存储 & ORM |
| 缓存 | Redis + Lettuce + Redisson 3.27.2 | 多级缓存 / 分布式锁 / GEO / BitMap / Stream |
| AI | Spring AI 1.0.0 | 智能问答 / RAG / 会话记忆 / 工具调用 |
| 前端 | Vue 3 + Vite 6 + Element Plus + Pinia | SPA（nginx 静态部署 + 反向代理 8080） |
...
│   ├── MessageController.java     #   消息中心（评论/关注通知）
...
src/main/resources/
├── application.yaml               # 主配置（端口、AI 模型、MyBatis）
├── application-dev.yaml           # 开发环境（本机 MySQL/Redis，开箱即用）
├── application-prod.yaml          # 生产环境（全环境变量注入）
├── db/hmdp.sql                    # 数据库建库建表 + 初始数据
├── db/migration_v2_db_optimize.sql # 数据库优化迁移脚本
├── knowledge/店铺信息.md           # RAG 知识库文档
├── prompts/system-prompt.st       # AI 系统提示词
├── mapper/VoucherMapper.xml       # MyBatis XML 映射
├── seckill.lua                    # 秒杀原子校验/扣减 Lua 脚本
