...
- **AI 探店助手增强**：集成 Spring AI + RAG 知识库 + Redis 会话记忆，支持流式 SSE 输出
- **UI/UX 优化**：
  - 首页瀑布流卡片支持图片原始比例自适应（与详情页一致）
  - 博客详情页标题显示 + 长标题省略
  - 店铺详情页卡片居中修复
  - 全局设计系统（tokens.css）+ 玻璃拟态主题
- **性能优化**：nginx gzip 压缩（主包 -62%）、图标按需注册（293 → 25 个）
- **架构改进**：CSS 架构重构（页面级样式隔离）、组件化（EmptyState/LikeIcon）