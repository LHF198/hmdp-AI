            ...
            if (!reader) {
              ...
              try {
                ...
                if (jsonResp.success === false && jsonResp.errorMsg) {
                  this.messages[messageIndex].content = "抱歉，AI服务暂时不可用：" + jsonResp.errorMsg;
                } else {
              ...
              while (true) {
                ...
                if (done) {
                  ...
                  if (buffer.startsWith('{"success":false')) {
                    try {
                      var jsonErr = JSON.parse(buffer);
                      if (jsonErr.errorMsg) {
                        this.messages[messageIndex].content = "抱歉，AI服务暂时不可用：" + jsonErr.errorMsg;
                      }
                    } catch (e) {
                      // 保持原内容
                    }
